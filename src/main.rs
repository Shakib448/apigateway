use api_gateway::{
    ...
    services::{cache_response, check_rate_limit, get_cached_response, is_authenticated},
...
async fn main() {
    ...
    let health_check = warp::path("health").and(warp::get()).map(|| "OK");
    ...
    let routes = health_check.or(proxy).recover(handle_rejection);
